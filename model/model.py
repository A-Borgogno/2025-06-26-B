...
class Model:
    ...
    def getGraphDetails(self):
        ...
        res = []
        for c in connessa:
            res.append((c, self._getMinEdge(c)))
    ...
    def _getMinEdge(self, nodo):
        return min(self._graph.get_edge_data(nodo, i)['weight'] for i in self._graph.neighbors(nodo))

    def _getNodi(self, minY, maxY):
        allCircuits = DAO.getAllCircuits()
        for c in allCircuits:
            for y in range(minY+1, maxY): #estremi esclusi
